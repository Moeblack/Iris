...
// extensions/telegram/node_modules/grammy/out/filter.js
...
// extensions/telegram/node_modules/grammy/out/context.js
...
// extensions/telegram/node_modules/grammy/out/composer.js
...
// extensions/telegram/node_modules/ms/index.js
...
// extensions/telegram/node_modules/debug/src/common.js
...
// extensions/telegram/node_modules/debug/src/browser.js
...
// extensions/telegram/node_modules/debug/src/node.js
...
// extensions/telegram/node_modules/debug/src/index.js
...
// extensions/telegram/node_modules/grammy/out/platform.node.js
...
// extensions/telegram/node_modules/grammy/out/core/error.js
...
// extensions/telegram/node_modules/webidl-conversions/lib/index.js
...
// extensions/telegram/node_modules/whatwg-url/lib/utils.js
...
// extensions/telegram/node_modules/tr46/lib/mappingTable.json
...
// extensions/telegram/node_modules/tr46/index.js
...
// extensions/telegram/node_modules/whatwg-url/lib/url-state-machine.js
...
// extensions/telegram/node_modules/whatwg-url/lib/URL-impl.js
...
// extensions/telegram/node_modules/whatwg-url/lib/URL.js
...
// extensions/telegram/node_modules/whatwg-url/lib/public-api.js
...
// extensions/telegram/node_modules/node-fetch/lib/index.js
...
// extensions/telegram/node_modules/@grammyjs/types/mod.js
var exports_mod = {};

// extensions/telegram/node_modules/grammy/out/types.node.js
...
// extensions/telegram/node_modules/grammy/out/types.js
...
// extensions/telegram/node_modules/grammy/out/core/payload.js
...
// extensions/telegram/node_modules/event-target-shim/dist/event-target-shim.js
...
// extensions/telegram/node_modules/abort-controller/dist/abort-controller.js
...
// extensions/telegram/node_modules/grammy/out/shim.node.js
...
// extensions/telegram/node_modules/grammy/out/core/client.js
...
// extensions/telegram/node_modules/grammy/out/core/api.js
...
// extensions/telegram/node_modules/grammy/out/bot.js
...
// extensions/telegram/node_modules/grammy/out/convenience/constants.js
...
// extensions/telegram/node_modules/grammy/out/convenience/inline_query.js
...
// extensions/telegram/node_modules/grammy/out/convenience/input_media.js
...
// extensions/telegram/node_modules/grammy/out/convenience/keyboard.js
...
// extensions/telegram/node_modules/grammy/out/convenience/session.js
...
// extensions/telegram/node_modules/grammy/out/convenience/frameworks.js
...
// extensions/telegram/node_modules/grammy/out/convenience/webhook.js
...
// extensions/telegram/node_modules/grammy/out/mod.js
...
// extensions/telegram/src/commands.ts
...
var TELEGRAM_MESSAGE_MAX_LENGTH = 4096;
function buildTelegramSessionTarget(params) {
  const scope = params.isPrivate ? "dm" : "group";
...
class TelegramClient {
  ...
  constructor(config) {
    this.config = config;
    this.bot = new import_grammy.Bot(config.token);
  }
  getBot() {
    return this.bot;
  }
  ...
  async start() {
    this.bot.start({
      onStart: (info) => {
        logger2.info(`已连接 | Bot: ${info.username}`);
      }
    });
    try {
      await this.bot.api.setMyCommands(TELEGRAM_BOT_COMMANDS);
      logger2.info(`已注册 Telegram 命令菜单 (${TELEGRAM_BOT_COMMANDS.length} 条)`);
    } catch (err) {
      logger2.warn("注册 Telegram 命令菜单失败:", err);
    }
  }
  async stop() {
    this.bot.stop();
  ...
  async sendPhoto(target, photo, caption) {
    const extra = {};
    if (target.threadId != null) {
      extra.message_thread_id = target.threadId;
    }
...
class TelegramMediaService {
  ...
  async downloadPhoto(client, photo) {
    try {
      ...
      logger3.debug(`图片下载成功: fileId=${photo.fileId}, size=${downloaded.buffer.length}`);
      return { mimeType, data: base64 };
    } catch (err) {
      logger3.error(`图片下载失败: fileId=${photo.fileId}`, err);
      return null;
    }
  }
  async downloadDocument(client, doc) {
    try {
      ...
      const base64 = downloaded.buffer.toString("base64");
      logger3.debug(`文件下载成功: fileId=${doc.fileId}, fileName=${fileName}, size=${downloaded.buffer.length}`);
      return { fileName, mimeType, data: base64 };
    } catch (err) {
      logger3.error(`文件下载失败: fileId=${doc.fileId}`, err);
      return null;
    }
  }
  async downloadVoice(client, voice) {
    try {
      ...
      const base64 = downloaded.buffer.toString("base64");
      logger3.debug(`语音下载成功: fileId=${voice.fileId}, duration=${voice.duration ?? "?"}s, size=${downloaded.buffer.length}`);
      return { fileName, mimeType, data: base64 };
    } catch (err) {
      logger3.error(`语音下载失败: fileId=${voice.fileId}`, err);
      return null;
    }
  }
}
...
// extensions/telegram/src/message-builder.ts
...
class TelegramMessageHandler {
  ...
  parseIncomingText(ctx) {
    ...
    if (chatId == null)
      return null;
    const username = String(ctx.me?.username ?? "").trim();
    ...
    const document2 = normalizeDocumentRef(message.document);
    ...
    if (!text && !photo && !document2 && !voice && !audio) {
      return null;
    }
  ...
  isExplicitlyMentioned(message, text, username) {
    if (isBlankTelegramUsername(username)) {
      logger4.debug("当前 update 未拿到 bot username，暂时放行群聊消息");
      return true;
    }
...
function normalizeDocumentRef(document2) {
  if (!document2?.file_id)
    return;
  return {
    fileId: document2.file_id,
    fileUniqueId: document2.file_unique_id,
    fileName: document2.file_name,
    mimeType: document2.mime_type
  };
}
function normalizeVoiceRef(voice) {
  if (!voice?.file_id)
    return;
  return {
...
function extractReplyRef(reply) {
  if (!reply)
    return;
  return {
...
function isBlankTelegramUsername(username) {
  return !username.trim();
}
function stripBotMention(text, username) {
  const normalized = text.trim();
  if (!normalized || isBlankTelegramUsername(username))
    return normalized;
...
class TelegramPlatform extends PlatformAdapter {
  ...
  async start() {
    ...
    if (this.pairingGuard && this.pairingStore?.needsBootstrap()) {
      ...
      logger5.info("");
      logger5.info("╔════════════════════════════════════════════════════════╗");
      logger5.info("║  首次使用，请在 Telegram 发送以下对码完成管理员绑定：    ║");
      logger5.info(`║  对码：${code}                                          ║`);
      logger5.info("║  第一个完成对码的用户将成为管理员。                      ║");
      logger5.info("╚════════════════════════════════════════════════════════╝");
      logger5.info("");
    }
  }
...
${activeLines}` : activeLines : cs.stream.buffer;
    ...
    try {
      ...
      if (messageDate > 0) {
        ...
        if (age > MESSAGE_EXPIRE_MS) {
          logger5.debug(`跳过过期消息: message_id=${parsed.messageId} (age=${Math.round(age / 1000)}s)`);
          return;
        }
      }
  ...
  async handleCallbackQuery(ctx) {
    try {
      ...
      if (!data || !chat || !messageId || !callbackQueryId)
        return;
      ...
      logger5.error("处理 callback query 时出错:", err);
    }
  }
  async handleCommand(text, cs) {
    ...
    switch (cmd.name) {
      ...
      case "models": {
        if (cmd.args) {
          ...
          await this.client.sendMessageWithKeyboard(cs.target, `\uD83E\uDD16 当前模型：${currentModel?.modelName ?? "未知"}
选择要切换的模型：`, keyboard);
        }
        return true;
      }
      ...
      case "sessions": {
        if (cmd.args) {
          ...
          await this.client.sendMessageWithKeyboard(cs.target, "\uD83D\uDCCB 选择要切换的会话：", keyboard);
        }
        return true;
      }
      case "stop": {
...
`));
      ...
      case "modes": {
        ...
        if (cmd.args) {
          ...
          await this.client.sendMessageWithKeyboard(cs.target, "\uD83C\uDFAD 选择要切换的 Mode：", keyboard);
        }
        return true;
      }
      case "invite": {
...
`).trim();
    ...
    logger5.info(`[${cs.sessionId}] 合并 ${messages.length} 条缓冲消息发送`);
