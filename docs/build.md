...
路由逻辑：存在 `-p`、`--prompt`、`--session`、`--model` 等 CLI 特征参数，或存在非 `-` 开头的位置参数时，进入 CLI 模式（`src/cli.ts`）；否则进入平台服务模式（`src/index.ts`）。
...
`data/` 目录包含配置模板。二进制通过 `import.meta.url` 计算 `projectRoot`（向上一级），在用户首次运行时将 `data/configs.example/` 复制到 `~/.iris/configs/` 完成初始化。
...
npm 包名为 **`irisagent`**，注册的命令为 **`iris`**。

### 包结构

```
irisagent (包装器包，npm install -g irisagent)
...
2. 生成 `irisagent` 包装器包（含启动器、postinstall、`data/`、`optionalDependencies` 指向各平台包）
3. `npm publish` 所有平台包
4. `npm publish` 包装器包
...
从 [GitHub Release](https://github.com/Lianues/IrisAgent/releases) 下载平台对应的压缩包，解压后直接执行 `bin/iris`（或 `bin/iris.exe`）。单文件，无外部依赖。
