...
| `plugins.yaml` | 否 | 插件配置（加载顺序、启用开关、插件自定义配置） |
...
| `summary.yaml` | 否 | 自动上下文压缩配置（/compact 指令） |
...
| `system.retryOnError` | `true` |
| `system.maxRetries` | `3` |
| `system.stream` | `true` |
| `system.maxAgentDepth` | `3` |
| `system.asyncSubAgents` | `false` |
